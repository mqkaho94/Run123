...
import time
import threading
import psycopg2  # 💡 由 sqlite3 改為 psycopg2
from psycopg2.extras import DictCursor # 💡 讓查詢結果能像原本的 sqlite 一樣用欄位名稱讀取
import os
import json
from datetime import date

# 1. 設定你的 Bot 憑證 (建議改用環境變數，或直接貼在後方當預設值)
TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "8999179825:AAGMP7VHxI75FniZG8KKv6XsJsuMfcSwudM")
BOT_USERNAME = "@Run1234567bot"

# 2. 💡 貼上你剛剛修改好（拿掉中括號、補上密碼）的 Supabase 連線字串
SUPABASE_URI = os.getenv("SUPABASE_DB_URI", "在這裡貼上你的_postgresql://postgres.xxxxxx:你的密碼@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres")
...
def get_db_connection():
    ...
    return psycopg2.connect(SUPABASE_URI, cursor_factory=DictCursor)
            ...
            with get_db_connection() as conn:
                with conn.cursor() as c:
                    ...
                    if row:
                        ...
                        to_username = row['username'] if row['username'] else target_username
                    else:
...
def run_race(chat_id):
    ...
    while (len(finished_horses) < 3) and (len(finished_horses) + len(dead_horses) < len(current_horses)):
        ...
        for h in current_horses:
            if horse_statuses[h]["dead_reason"] or current_distance[h] >= TOTAL_DISTANCE: continue
            ...
            if horse_statuses[h]["is_debuff_carrier"]: step_modifier = 0.1
                ...
                step_modifier = 2.0 if move_roll >= 9 else 1.0 if move_roll >= 4 else 0.5
                
            current_distance[h] = min(TOTAL_DISTANCE, current_distance[h] + (speeds[h] * step_modifier) + random.uniform(-0.2, 0.2))
            
            if current_distance[h] >= TOTAL_DISTANCE and h not in finished_horses:
    ...
    try: bot.edit_message_text("🏁 **比賽結束！正在計算最終名次與分紅...**", chat_id, race_msg.message_id, parse_mode='Markdown')
    except: pass
...
def money(message): 
    sync_username(message.from_user.id, message.from_user.username)
    bot.reply_to(message, f"💰 你的餘額：**{get_chips(message.from_user.id):,}** 金幣", parse_mode='Markdown')
...
# ================== 啟動服務 ==================
print(f"🚀 {BOT_USERNAME} 【Supabase 雲端資料庫版】啟動！")
